...
通用 Linux **用户态** Python 驱动，通过 libgpiod 2.x 操作 GPIO，软件模拟 4-wire SPI Mode 0、MSB first。面向首次点屏，不需要 spidev 或专用内核模块。默认照片画布为横向 720×480；按参考代码保留底层 480×720 扫描帧，通过旋转转换横屏。六色、4bpp，一帧 172800 字节。
...
原始传输帧逐行 480 像素、共 720 行（不是横屏照片画布）；每字节高半字节为前一个像素、低半字节为后一个像素。色码：黑 0、白 1、黄 2、红 3、蓝 5、绿 6。例如 `0x31` 为红/白两个像素。程序在访问硬件前检查长度及色码。Web 横屏图像先处理为 720×480，再逆时针旋转 90° 映射到该原始帧；预览按相反方向还原。竖屏选项直接使用 480×720。物理安装方向及扫描顺序仍需实屏确认；不能仅改变行宽来替代旋转。
