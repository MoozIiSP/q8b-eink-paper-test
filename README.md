...
通用 Linux **用户态** Python 驱动，通过 libgpiod 2.x 操作 GPIO，软件模拟 4-wire SPI Mode 0、MSB first。面向首次点屏，不需要 spidev 或专用内核模块。屏幕固定为 480×720，六色、4bpp，一帧 172800 字节。

## Web 管理与新算法

已增加[Web 管理端](docs/web-manager.md)：上传图片、六色预览、刷新/刷白/色条与任务状态。支持无抖动和 Floyd–Steinberg 图像转换；刷新策略加入成功帧去重、持久化 150 秒间隔及串行执行，GPIO 传输减少同控制器上的写入调用。面板仍使用原厂全刷波形，不宣称支持快速局刷。
...
逐行 480 像素、共 720 行；每字节高半字节为前一个像素、低半字节为后一个像素。色码：黑 0、白 1、黄 2、红 3、蓝 5、绿 6。例如 `0x31` 为红/白两个像素。程序在访问硬件前检查长度及色码。方向与扫描顺序需首板色条/图片实测，当前按照示例 480×720 顺序发送。
